...
from .sd15_quant_config import w8_uniform_config, a8_mixed_precision_config
...
def _add_observer_(module, qconfig_propagation_list=None, non_leaf_module_list=None,
                   device=None, custom_module_class_mapping=None):
    ...
    for name, child in module.named_children():
        if type_before_parametrizations(child) in [nn.Dropout]:
            ...
            _add_observer_(child, qconfig_propagation_list, non_leaf_module_list,
...
def _remove_activation_post_process(module):
    ...
    def remove_hooks(pre_hook=False):
        ...
        handle_ids_to_remove = {
            hid for hid, fn in hook_map.items() if fn is observer_hook
        }
...
def prepare(model, inplace=False, allow_list=None,
    ...
    _add_observer_(model, qconfig_propagation_list, observer_non_leaf_module_list,
    ...
    if hasattr(mod, 'qconfig') and mod.qconfig is not None:
        ...
        if type_before_parametrizations(mod) in custom_module_class_mapping:
            new_mod = custom_module_class_mapping[
                type_before_parametrizations(mod)].from_observed(mod)
            swapped = True
...
def convert(module, mapping=None, inplace=False, remove_qconfig=True,
    ...
    if not inplace:
        module = copy.deepcopy(module)
    _convert(module, mapping, inplace=True, is_reference=is_reference,
...
class QParam(namedtuple("QParam", ["qscheme", "dtype", "scales", "zero_points", "axis"],
...
def get_quant_para(ckpt, n_bit, module_name, quant_type, split=0, device=None):
    ...
    return (scales.to(device), zero_points.to(device),
            scales_0.to(device), zero_points_0.to(device))


def create_qparams_from_dtype(dtype, device, is_channel_wise=False, num_kernels=None,
                               ckpt=None, module_name=None, bit_width=0,
                               quant_type=None, split=0):
    if dtype == torch.float16:
        return None

    elif dtype in [torch.qint8, torch.quint8, torch.quint4x2]:
        if quant_type == 'weight':
            scales, zero_points, scales_0, zero_points_0 = get_quant_para(
                ckpt, bit_width, module_name, quant_type='weight', split=split, device=device)
        elif quant_type == 'act':
            scales, zero_points, scales_0, zero_points_0 = get_quant_para(
                ckpt, bit_width, module_name, quant_type='act', split=split, device=device)
    else:
        raise ValueError(f"Unsupported quantize dtype {dtype}")

    if is_channel_wise:
        assert num_kernels is not None
        qparam = QParam(qscheme=torch.per_channel_affine, scales=scales,
                        zero_points=zero_points, dtype=dtype, axis=0)
        qparam_0 = QParam(qscheme=torch.per_channel_affine, scales=scales_0,
                          zero_points=zero_points_0, dtype=dtype, axis=0) if split > 0 else None
    else:
        qparam = QParam(qscheme=torch.per_tensor_affine, scales=scales,
                        zero_points=zero_points, dtype=dtype)
        qparam_0 = QParam(qscheme=torch.per_tensor_affine, scales=scales_0,
                          zero_points=zero_points_0, dtype=dtype) if split > 0 else None
...
def dequantize_to_float16(x: torch.Tensor, qparams: QParam):
    ...
    return dequantize_per_tensor_uint4(
        x, qparams.scales.to(x.device), qparams.zero_points.to(x.device)).to(torch.float16)
...
class QuantizedConv2d(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, kernel_size,
                 ...
                 device=None, w_qparams=None, w_qparams_0=None,
                 a_qparams=None, a_qparams_0=None,
                 module_name=None, split=0) -> None:
        super().__init__()
        ...
        if self.valid_for_acceleration and (
            ...
            logging.warning(
                f"Conv2d layer with in_channels={self.in_channels} and "
        ...
        self.valid_for_int8_storage = (
            w_qparams is not None and
            w_qparams.dtype in [torch.qint8, torch.quint8] and
            w_qparams.qscheme == torch.per_channel_affine
        )
    ...
    def from_float(cls, float_mod, split=0, ckpt=None):
        ...
        w_qparams, w_qparams_0 = create_qparams_from_dtype(
            dtype=w_dtype, device=device, is_channel_wise=True,
            num_kernels=num_kernels, ckpt=ckpt,
            module_name=float_mod.module_name, quant_type='weight',
            bit_width=float_mod.w_bit, split=split)

        act_process = float_mod.qconfig.activation()
        act_dtype = act_process.dtype

        if hasattr(float_mod, 'a_bit'):
            a_qparams, a_qparams_0 = create_qparams_from_dtype(
                dtype=act_dtype, device=device, is_channel_wise=False,
                num_kernels=num_kernels, ckpt=ckpt,
                module_name=float_mod.module_name, quant_type='act',
                bit_width=float_mod.a_bit, split=split)
        else:
            a_qparams = None
            a_qparams_0 = None

        new_mod = cls(
            float_mod.in_channels, float_mod.out_channels, float_mod.kernel_size,
            float_mod.stride, float_mod.padding, float_mod.dilation,
            float_mod.groups, float_mod.bias is not None,
            device=float_mod.weight.device,
            w_qparams=w_qparams, w_qparams_0=w_qparams_0,
            a_qparams=a_qparams, a_qparams_0=a_qparams_0,
            module_name=float_mod.module_name, split=split)
        ...
        if split == 0:
            if new_mod.valid_for_acceleration:
                weight_int = torch.quantize_per_channel(
                    weight.float(), new_mod.weight_scales, new_mod.weight_zero_points,
                    axis=w_qparams.axis, dtype=w_qparams.dtype).int_repr()
                ...
                weight_int = _quantize_weight_to_int8(
                    weight, new_mod.weight_scales, new_mod.weight_zero_points)
                new_mod.register_buffer("weight_int", weight_int)
            else:
            ...
            if new_mod.valid_for_acceleration:
                weight_int = torch.quantize_per_channel(
                    weight[:, :split, ...].float(), new_mod.weight_scales,
                    new_mod.weight_zero_points, axis=w_qparams.axis,
                    dtype=w_qparams.dtype).int_repr()
                weight_int_0 = torch.quantize_per_channel(
                    weight[:, split:, ...].float(), new_mod.weight_scales_0,
                    new_mod.weight_zero_points_0, axis=w_qparams_0.axis,
                ...
                weight_int = _quantize_weight_to_int8(
                    weight[:, :split, ...], new_mod.weight_scales, new_mod.weight_zero_points)
                weight_int_0 = _quantize_weight_to_int8(
                    weight[:, split:, ...], scales_0, zps_0)
                new_mod.register_buffer("weight_int", weight_int)
                new_mod.register_buffer("weight_int_0", weight_int_0)
    ...
    def forward_fallback(self, x: torch.Tensor):
        ...
        weight_recovered = self._dequantize_weight(
            self.weight_int, self.weight_scales, self.weight_zero_points).to(x.dtype)
        bias = self.bias.to(x.dtype) if self.bias is not None else None

        if self.split == 0:
            return F.conv2d(x, weight_recovered, bias,
                            self.stride, self.padding, self.dilation, self.groups)
        else:
            weight_0_recovered = self._dequantize_weight(
                self.weight_int_0, self.weight_scales_0, self.weight_zero_points_0).to(x.dtype)
            output = F.conv2d(x[:, :self.split, :, :], weight_recovered, bias,
                              self.stride, self.padding, self.dilation, self.groups)
            output_0 = F.conv2d(x[:, self.split:, :, :], weight_0_recovered, None,
                                self.stride, self.padding, self.dilation, self.groups)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.split == 0:
            ...
            return qconv2d(x_int, self.weight_int, self.weight_scales,
                           self.act_scales, self.act_zero_points, self.scale,
                           self.weight_sum_by_input_channels, self.bias0, self.bias,
                           self.stride[0], self.padding[0])
        else:
            x_int = quantize_per_tensor(x[:, :self.split, :, :],
                                        self.act_scales_inv, self.act_zero_points)
            x_int_0 = quantize_per_tensor(x[:, self.split:, :, :],
                                          self.act_scales_inv_0, self.act_zero_points_0)
            output = qconv2d(x_int, self.weight_int, self.weight_scales,
                             self.act_scales, self.act_zero_points, self.scale,
                             self.weight_sum_by_input_channels, self.bias0, self.bias,
                             self.stride[0], self.padding[0])
            output_0 = qconv2d(x_int_0, self.weight_int_0, self.weight_scales_0,
                               self.act_scales_0, self.act_zero_points_0, self.scale_0,
                               self.weight_sum_by_input_channels_0, self.bias0_0, None,
                               self.stride[0], self.padding[0])
            return output + output_0
...
class QuantizedLinear(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True,
        ...
        if self.valid_for_acceleration and (
            ...
            logging.warning(
                f"Linear layer with in_features={self.in_features} and "
    ...
    def from_float(cls, float_mod, split=0, ckpt=None):
        ...
        w_qparams, _ = create_qparams_from_dtype(
            dtype=w_dtype, device=device, is_channel_wise=True,
            num_kernels=num_kernels, ckpt=ckpt,
            module_name=float_mod.module_name, quant_type='weight',
            bit_width=float_mod.w_bit, split=split)
        ...
        if hasattr(float_mod, 'a_bit'):
            a_qparams, _ = create_qparams_from_dtype(
                dtype=act_dtype, device=device, is_channel_wise=False,
                num_kernels=num_kernels, ckpt=ckpt,
                module_name=float_mod.module_name, quant_type='act',
                bit_width=float_mod.a_bit, split=split)
        ...
        new_mod = cls(float_mod.in_features, float_mod.out_features,
                      float_mod.bias is not None, device=float_mod.weight.device,
                      w_qparams=w_qparams, a_qparams=a_qparams,
    ...
    def forward_fallback(self, x):
        ...
        return F.linear(x, weight_recovered,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if not hasattr(self, 'bos') or not self.bos:
            x_int = quantize_per_tensor(x, self.act_scales_inv, self.act_zero_points)
            return qlinear(x_int, self.weight_int, self.weight_scales,
                           self.act_scales, self.act_zero_points,
                           self.weight_sum_by_input_channels, self.scale,
                           self.bias0, self.bias)
        else:
            x_except_first_token = quantize_per_tensor(
                x[:, 1:, :], self.act_scales_inv, self.act_zero_points)
            out_except_first_token = qlinear(
                x_except_first_token, self.weight_int, self.weight_scales,
                self.act_scales, self.act_zero_points,
...
def register_qconfig_from_input_files(unet, w_bit=8, a_bit=None, bos=True, bos_dict=None):
    ...
    mod_name_to_weight_width = {
        filter_mod_name_prefix(k): v for k, v in mod_name_to_weight_width.items()
    }
    ...
    mod_name_to_act_width = {
        filter_mod_name_prefix(k): v for k, v in mod_name_to_act_width.items()
    }
...
def convert_to_quantized(unet, ckpt):
    convert(unet,
            mapping={nn.Linear: QuantizedLinear, nn.Conv2d: QuantizedConv2d},
            inplace=True,
            ckpt=ckpt)
    ...
    * Simpler denoising loop without SDXL-specific ``added_cond_kwargs``
    ...
    def check_inputs(
        ...
        if callback_steps is not None and (
            ...
            raise ValueError(
                f"`callback_steps` has to be a positive integer but is {callback_steps}.")
    ...
    def prepare_latents(self, batch_size, num_channels_latents, height, width,
        ...
        if isinstance(generator, list) and len(generator) != batch_size:
            raise ValueError(
                f"You passed {len(generator)} generators for batch size {batch_size}.")
    ...
    def quantize_unet(
        ...
        register_qconfig_from_input_files(
            self.unet, w_bit=w_bit, a_bit=a_bit, bos=bos, bos_dict=bos_dict)
